...
import semanticAnalyse.ErrorProduction;
import semanticAnalyse.Node;
import semanticAnalyse.Symbol;
...
public class GrammarComplier {
	...
	public List<String> getfourCodes() {
		return fourcodes;
	}
	...
	public GrammarComplier() {
		...
		fourcodes = new ArrayList<String>();
...
//										System.out.println(rr_symbol.getName()+right_symbol.getName());
	...
	// 返回能使name推出空的一连串产生式的第一个,如果name不能推出空，返回null
	...
	public List<Production> analysis(List<Token> token_list){
		...
		while(pos<token_list.size()){
			...
			//当leftest是一个代表语义分析程序段的文法符号时，left_node.getFather()就代表它所在产生式的左部结点
			//为父节点声明类型
			if(leftest.getName().equals("M68_2")){
				...
				Node father=left_node.getFather();
				//识别数组下标不是整数的错误
				if (token_list.get(pos-2).getValue().contains(".")){
					System.out.println("下标不为整数, 行数： " + line);
					break;
				}
				...
				if(father.attribute.get("length")!=null){//定义语句中的数组
					...
					int idflag = 1;
					for (int i=0;i<ids.size();i++){
						if (ids.get(i).name.equals(id.name)){
							System.out.println("重复定义的变量: "+id.name+" 行数: "+line);
							idflag = 0;
							break;
						}
					}
					...
					Id id=getId(name);
					if (id == null){
						System.out.println("未声明的变量: "+name+" 行数:" + line);
						break;
					}
			...
			else if(leftest.getName().equals("M15_6") || leftest.getName().equals("M63_3") || leftest.getName().equals("M61_4") || leftest.getName().equals("M62_2")){
				...
				if(inh==null || inh.equals("null"))
					...
					/*
					for(int i=fourcodes.size()-1;i>=0;i--){
						if(fourcodes.get(i)!=null&&fourcodes.get(i).startsWith(temp)){
							fourcodes.remove(i);
						}
					}
					 */
				...
				for (int i=0;i<ids.size();i++){
					if (ids.get(i).name.equals(inh)){
						if (value.contains(".")&&ids.get(i).type.equals("int")){
							System.out.println("赋值类型不匹配，行数： " + line);
							break;
						}
					}
				}
				codes.add(inh+" := "+value);
				fourcodes.add("(=, "+value+" ,- ,"+ inh +" )");
				...
				fourcodes.add("(jf, "+b+" ,- ,"+ (fourcodes.size()+2) +" )");
				father.attribute.put("backpatch", ""+codes.size());
				father.attribute.put("backpatchf", ""+fourcodes.size());
				codes.add(null);
				fourcodes.add(null);
			}
				...
				int backpatchf=Integer.parseInt(father.attribute.get("backpatchf"));
				
				codes.add(null);
				fourcodes.add(null);
				codes.set(backpatch, "goto "+codes.size());//回填
				fourcodes.set(backpatchf,"(j, - ,- ,"+ fourcodes.size() +" )");
				
				father.attribute.put("backpatch", ""+(codes.size()-1));
				father.attribute.put("backpatchf", ""+(fourcodes.size()-1));
			}
				...
				int backpatchf=Integer.parseInt(father.attribute.get("backpatchf"));
				int backtof=Integer.parseInt(father.attribute.get("backtof"));
				//入口
				codes.add("goto "+backto);
				fourcodes.add("(j, - ,- ,"+ backtof +" )");
				//出口
				codes.set(backpatch, "goto "+codes.size());//回填
				fourcodes.set(backpatchf,"(j, - ,- ,"+ fourcodes.size() +" )");
			}
				...
				father.attribute.put("backtof", ""+fourcodes.size());
			}
